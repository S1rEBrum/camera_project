...
void init_radio() {
  ...
  if (!radio.begin(hspi)) {
    ...
    Serial.println("Radio initialized successfully");  // Print success message if initialization is successful
  }
...
void save_image_to_sd(uint8_t *fb, int size) {
  ...
  if (!file) {
    Serial.println("Failed to open file in writing mode");  // Print error message if the file cannot be opened
  } else {
