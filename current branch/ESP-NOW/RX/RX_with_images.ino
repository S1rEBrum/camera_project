...
void write_file(uint8_t *fb) {
  if (SET_DEBUG) { Serial.println("Starting the writing to SD"); };

  String path = "/picture" + String(pictureNumber) + ".jpg";

  fs::FS &fs = SD;
  if (SET_DEBUG) { Serial.printf("Picture file name: %s\n", path.c_str()); };

  File file = fs.open(path.c_str(), FILE_WRITE);
  if (!file) {
    Serial.println("Failed to open file in writing mode");
  } else {
    if (SET_DEBUG) { Serial.printf("Size of frame buffer is : %d\n", image_size); };
